...
export default {
   ...
   * }
    ...
    setPrices: function () {
        var tickers = [];
        var tickersList = JSON.parse(localStorage.getItem('tickers')) || {};

        for (let item in tickersList) {
            tickersList[item]['market'] = item;
            tickers.push(tickersList[item]);
        }
        ...
        for (let base of noExit) {
            for (let b of defaultCoins) {
                if (lastPrices[base + '_' + b]) {
                    price[base] = parseFloat(lastPrices[base + '_' + b]);
                    break;
                }
            }
        }
    ...
    toCNY(market, balance) {
        if (!market || !balance || market.indexOf('--') != -1 ) {
            return 0;
        }
        this.setPrices();
        ...
        if (market.indexOf('_') == -1 && market.indexOf('/') == -1) { //单个币种
            ...
            if (base && prices && prices[base]) price = prices[base];
        }
    ...
    toC2C: function (coins, data, len) {
        var lastPrices = JSON.parse(localStorage.lastPrices || '{}') || {};
        var tempData = {};
        ...
        for (var coin in tempData) {
            var price = lastPrices['C2C_' + coin] || lastPrices['CNN_' + coin] || 0;
            if (price == 0) {
                if (coin == 'C2C' || coin == 'CNN') {
                    price = 1;
                }
                else if (coin == 'BTC')
